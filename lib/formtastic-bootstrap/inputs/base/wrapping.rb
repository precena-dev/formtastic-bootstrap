    ...
    module Base
      module Wrapping
        ...
        def bootstrap_wrapping(&block)
          ...
          form_wrap_class = 'form-wrapper'
          form_wrap_class = form_wrap_class + ' ' + options[:form_wrap_class] if options[:form_wrap_class]

          form_group_wrapping do
            label_html <<
                template.content_tag(:span, :class => form_wrap_class) do
                  input_content(&block) <<
                      hint_html(:block) <<
                      error_html(:block)
                end
